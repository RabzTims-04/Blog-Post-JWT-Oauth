...
blogsRouter
  ...
  .post(uploadOnCloudinary, async (req, res, next) => {
    try {
      ...
      const updatedCover = await blogModel.findByIdAndUpdate(blogId, newCover, {
        new: true
      })
      if(!updatedCover){
        return next(createError(404, `Blog with id ${blogId} not found`))
      }else{
...
blogsRouter.put("/:blogId", JWTAuthMiddleware, async (req, res, next) => {
  try {
    ...
    if(blog.authors.includes(authorId)){
      ...
      next(createError(403, "You have no access to edit this post"))
    }    
  } catch (error) {
    next(
      createError(
...
blogsRouter.delete("/:blogId", JWTAuthMiddleware, async (req, res, next) => {
  try {
    ...
    if(blog.authors.includes(authorId)){
      ...
      next(createError(403, "You have no access to delete this post"))
    } 
  } catch (error) {
    next(
      createError(
